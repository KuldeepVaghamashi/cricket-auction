...
/** Isolated countdown timer — its 1-second tick only re-renders this badge. */
function CountdownBadge({ startsAtMs, isDraft }: { startsAtMs: number; isDraft: boolean }) {
  ...
  const ms = Number.isFinite(startsAtMs) ? startsAtMs - nowMs : -1;
  if (ms <= 0) {
    return (
...
export default function AuctionViewerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Memoized lookup maps — rebuilt only when the underlying data changes,
  // not on every render tick.
  const inactiveTeamNameById = useMemo(
    () => new Map((allTeams ?? []).map((t) => [t._id, t.name])),
    [allTeams]
  );
  const activeTeamNameById = useMemo(
    () =>
      new Map(
        (streamData?.teams ?? [])
          .filter((t) => t._id)
  ...
  const recentBidCounts = useMemo(() => {
    ...
    for (const b of streamData?.state?.bidHistory ?? []) {
  ...
  if (auctionMeta.status !== "active") {
    const teamNameById = inactiveTeamNameById;
          ...
          <Card className="w-full max-w-4xl">
            ...
            <CardContent>
              <div className="flex flex-col items-center text-center gap-3">
                ...
                <CountdownBadge startsAtMs={startsAtMs} isDraft={isDraft} />
  ...
  const teamNameById = activeTeamNameById;
  const soldPlayers = (allPlayers ?? []).filter((p) => p.status === "sold");
